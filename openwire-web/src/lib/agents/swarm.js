...
export class AgentSwarm {
    ...
    async _processQueue() {
        ...
        try {
            ...
            // ── Build system prompt via shared builder (single source of truth) ──
            // The same buildSystemPrompt() is used by the prompt-audit harness,
            // so the audit exercises the exact prompt production runs.
            const mood = this._moods[characterId];
            const systemPrompt = buildSystemPrompt(c, {
                mood,
                sessionFacts: this._sessionFacts,
                contextSummary: this._contextSummary,
                guardrails: this._guardrails,
                taskPrompt: this._buildTaskPrompt(characterId),
            });
